...
public class PSMEval extends PSM {
	...
	/*
	 * @Override public void exec(PSMInterp interp, Map<String, Set<Map<Object,
	 * Object>>> state) { try { Quad<Instance, Map<Node, Map<Object,
	 * Transform>>, Map<Node, Triple<Instance, Map<Object, Pair<Object,
	 * Object>>, Map<Pair<Object, Object>, Object>>>, Pair<Map<Node,
	 * Triple<Instance, Map<Object, Path>, Map<Path, Object>>>, Map<Edge,
	 * Transform>>> xxx = interp.exps.get(AB); Instance ab = xxx.first;
	 * Map<Node, Map<Object, Transform>> map = xxx.second; Map<Node,
	 * Triple<Instance, Map<Object, Pair<Object, Object>>, Map<Pair<Object,
	 * Object>, Object>>> map2 = xxx.third; Pair<Map<Node, Triple<Instance,
	 * Map<Object, Path>, Map<Path, Object>>>, Map<Edge, Transform>> map3 =
	 * xxx.fourth;
	 * 
	 * Instance abb = new Instance(sig, PSMGen.gather(ABB, sig, state));
	 * //Instance ab = new Instance(sig, PSMGen.gather(AB, sig, state));
	 * //already have in interp Instance a = new Instance(sig, PSMGen.gather(A,
	 * sig, state)); Instance b = new Instance(sig, PSMGen.gather(B, sig,
	 * state));
	 * 
	 * Transform fst = new Transform(abb, ab, PSMGen.gather(ABB + "_fst", sig,
	 * state)); Transform snd = new Transform(abb, b, PSMGen.gather(ABB +
	 * "_snd", sig, state));
	 * 
	 * List<Pair<String, List<Pair<Object, Object>>>> data = new LinkedList<>();
	 * for (Node n : sig.nodes) { List<Pair<Object, Object>> d = new
	 * LinkedList<>(); Map<Object, Transform> m = map.get(n); Triple<Instance,
	 * Map<Object, Pair<Object, Object>>, Map<Pair<Object, Object>, Object>> m2
	 * = map2.get(n); Map<Path, Object> m3 = map3.first.get(n).third;
	 * 
	 * for (Pair<Object, Object> id : abb.data.get(n.string)) { Object id_ab =
	 * lookup(fst.data.get(n.string), id.first); Object x =
	 * lookup(snd.data.get(n.string), id.first); Transform t = m.get(id_ab);
	 * //don't forget to apply of to any paths // System.out.println(m2.third);
	 * // System.out.println("n " + n); // System.out.println("x " + x); Object
	 * y = m3.get(sig.toCategory2().second.of(new Path(sig, n)).arr); //
	 * System.out.println(y); Object p = m2.third.get(new Pair<>(x, y)); Object
	 * f = lookup(t.data.get(n.string), p); d.add(new Pair<>(id.first, f)); }
	 * 
	 * data.add(new Pair<>(n.string, d)); } Transform curry = new Transform(abb,
	 * a, data); PSMGen.shred(pre, curry, state);
	 * 
	 * } catch (FQLException fe) { fe.printStackTrace(); throw new
	 * RuntimeException(fe.getMessage()); } }
	 */
	...
	public void exec(PSMInterp interp,
		...
		try {
			...
			Transform fst = new Transform(abb, Jw, PSMGen.gather(ABB + "_fst",
					sig, state));
			Transform snd = new Transform(abb, b, PSMGen.gather(ABB + "_snd",
					sig, state));
			...
			for (Node n : sig.nodes) {
				...
				for (Pair<Object, Object> id : abb.data.get(n.string)) {
					...
					LinkedHashMap<Pair<Arr<Node, Path>, Attribute<Node>>, Object> w = m2
							.get(id_ab).first;
	...
	public PSMEval(String pre, String a, String b, String aB, String aBB,
			Signature sig) {
		super();
		this.pre = pre;
		A = a;
