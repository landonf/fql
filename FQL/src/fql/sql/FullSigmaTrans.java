...
public class FullSigmaTrans extends PSM {
	...
	public void exec(PSMInterp interp,
		...
		//TODO remove src, dst if not necessary
//		List<Pair<String, List<Pair<Object, Object>>>> I0X = PSMGen.gather(src, D, state);
...
//			Instance IX = new Instance(D, I0X);
			...
			Triple<Instance, Map<Node, Map<Integer, Integer>>, Map<Node, Map<Integer, Integer>>> xxx = Denotation.fullSigmaWithAttrs(interp, f, I, HX, JX, interp.sigmas2.get(src));
			interp.guid = current;
			...
			for (Attribute<Node> a : D.attrs) {
				state.put(pre + "_" + a.name, new HashSet<Map<Object, Object>>());
			}
			for (Edge a : D.edges) {
				state.put(pre + "_" + a.name, new HashSet<Map<Object, Object>>());
			}
