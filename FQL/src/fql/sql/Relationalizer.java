...
public class Relationalizer {
	...
	public static List<PSM> compile(Signature sig, String out, String in) throws FQLException {
...
//				}
				...
				ret.add(new CreateTable(n.string + "tempNoAttrs" + count, edge_types, false));
				InsertSQL f0 = new InsertSQL(n.string + "tempNoAttrs" + count, f);
				ret.add(f0);
			...
			for (int i = 0; i < count; i++) {
				...
				for (int v = 0; v < types.size(); v++) {
					ty.put("c" + u, types.get("c" + v));
					select.put("c" + u, new Pair<>(n.string + "temp" + i, "c1"));
					u++;
				}
			...
			if (select.size() == 0) {
				throw new RuntimeException("No observable for " + n.string + " in " + sig.name0);
			}
			...
			ret.add(new InsertKeygen(n.string + "_observables_guid", "id", n.string + "_observables_proj", new LinkedList<>(ty.keySet())));
			
			select = new HashMap<>();
			where = new LinkedList<>();
			from = new HashMap<>();
			from.put(n.string + "_observables", n.string + "_observables");
			from.put(n.string + "_observables_guid", n.string + "_observables_guid");
			for (int u0 = 0; u0 < u; u0++) {
				where.add(new Pair<>(new Pair<>(n.string + "_observables", "c" + u0),
	...
	private static Collection<PSM> applySubst(Signature sig, Node N,
			String out) {
		List<PSM> ret = new LinkedList<>();
		...
		ret.add(new InsertSQL(out + "_" + N.string, new CopyFlower("relationalize_temp")));
		...
		for (Edge n : sig.edges) {
			...
			where.add(new Pair<>(new Pair<>(N + "_subst", "c0"), new Pair<>(out + "_" + n.name, "c0")));
			select.put("c0", new Pair<>(N + "_subst", "c1"));
			...
			ret.add(new InsertSQL(out + "_" + n.name, new CopyFlower("relationalize_temp")));
		...
		for (Edge n : sig.edges) {
			...
			from.put(N + "_subst", N + "_subst");
			from.put(out + "_" + n.name, out + "_" + n.name);
			where.add(new Pair<>(new Pair<>(N + "_subst", "c0"), new Pair<>(out + "_" + n.name, "c1")));
			select.put("c1", new Pair<>(N + "_subst", "c1"));
	...
	private static List<PSM> copy(Signature sig, String out, String in) {
		...
		for (Attribute<Node> n : sig.attrs) {
			...
			ret.add(new CreateTable(out + "_" + n.name, attrs, false));
			ret.add(new InsertSQL(out + "_" + n.name, new CopyFlower(in + "_" + n.name)));
		}
		for (Edge n : sig.edges) {
			Map<String, String> attrs = new HashMap<>();
			attrs.put("c0", PSM.VARCHAR());
